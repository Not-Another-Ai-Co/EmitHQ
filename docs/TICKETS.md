...
### T-027: Fix Deployment Blockers & Deploy API to Railway [x]

**Phase:** 7
**Effort:** Medium
**Complexity:** Moderate
**Depends on:** none
**Research:** docs/research/gtm-execution.md, docs/tmp/research-deployment-readiness.md

**Description:** Fix the P0 blockers preventing deployment: create worker entry point (startDeliveryWorker is never called), generate Drizzle migrations, add missing tsconfig.json for packages/api, create railway.toml for multi-service deployment. Deploy API + worker to Railway with environment variables from .env.tpl.

**Acceptance criteria:**

- [x] Worker entry point created (`packages/api/src/worker.ts`) — calls `startDeliveryWorker()` with graceful shutdown
- [x] Production runtime resolved — tsx used instead of tsc+node (DEC-021: avoids path alias and TS source export issues)
- [x] Drizzle migrations generated (`0000_narrow_moonstone.sql`) and npm scripts wired (`db:generate`, `db:migrate`, `db:push`, `db:studio`)
- [-] `railway.toml` with web + worker service configuration (DEVIATION: Railway dashboard config is simpler for shared monorepo with external DB/Redis — no railway.toml needed)
- [ ] Schema applied to Neon production database (`npm run db:migrate` — Julian to run with DATABASE_ADMIN_URL)
- [ ] Missing 1Password items created: `sentry` (dsn), `metrics` (secret) — Julian to input values
- [ ] Environment variables set in Railway from .env.tpl references — Julian to configure in Railway dashboard
- [ ] API server running on Railway with `GET /health` returning 200
- [ ] End-to-end smoke test: create app → create endpoint → send message → verify delivery attempt logged
